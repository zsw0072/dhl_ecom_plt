...
# def insertData(month):
with open("C:\\Users\\admin\\Desktop\\dhl_ecom_plt\\orgin\\CAO_201912ecom_base.xlsx", 'r', encoding='GBK') as f:
       print(f.readlines(1024))
    ...
    #     c.execute("INSERT INTO ecom_base_"+month+" VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, \
    #               ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
