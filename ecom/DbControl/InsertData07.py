...
for curr_row in range(nrows):
    ...
    c.execute("INSERT INTO ecom_base_201907 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, \
              ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
              (sd_dtm,awb_no,orig_ctry,orig_stn,orig_fclty,cleaned_orig_city_name,sd_route_id,dest_ctry,
               ...
               message_create_dtm_ecom,Montime,exemptEFR,cleaned_product_code,billing_acct_no_ctry,ezsite_authorizing,extra_charge,PLT))
